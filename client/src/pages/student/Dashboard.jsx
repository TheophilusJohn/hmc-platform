...
import { PageWrapper, Card, StatCard, Badge, Btn, Timeline } from '../../components/common';
  ...
  return (
    <PageWrapper title={`Welcome, ${user?.firstName || 'Student'}`} subtitle="Your academic overview">
      <div style={{ display: 'flex', gap: 16, marginBottom: 20, flexWrap: 'wrap' }}>
        ...
        <StatCard icon="📅" label="Attendance" value={`${stats?.attendance || 0}%`} color={stats?.attendance < 75 ? '#991B1B' : '#0F766E'} />
          ...
          {(upcoming?.exams || []).length === 0 ? (
            ...
            <div key={e.id} style={{ padding: '12px 0', borderBottom: '1px solid #DDE1E7', display: 'flex', gap: 12, alignItems: 'center' }}>
              ...
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: '#0F2B4A' }}>{new Date(e.startTime).toLocaleDateString('en-IN')}</div>
                <div style={{ fontSize: 11, color: '#7B8494' }}>{new Date(e.startTime).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}</div>
              </div>
