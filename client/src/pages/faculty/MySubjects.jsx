...
export default function MySubjects() {
  ...
  // Note: studentCount and attendanceAvg were rendered here but the server
  // /subjects?mine=true endpoint doesn't return either, so every row showed
  // "0 / —" misleadingly. Surfacing real numbers needs an aggregate query
  // server-side — deferred to a future enhancement (audit-fresh §13).
  const cols = [
    { key: 'code', label: 'Code', render: v => <code style={{ background: '#EEF4FA', padding: '2px 6px', borderRadius: 4, fontSize: 12, color: '#0F2B4A' }}>{v}</code> },
    { key: 'name', label: 'Subject', render: (v, r) => <div><div style={{ fontWeight: 500 }}>{v}</div><div style={{ fontSize: 12, color: '#7B8494' }}>{[r.batchName, r.semesterName].filter(Boolean).join(' · ')}</div></div> },
    { key: 'creditHours', label: 'Credits', render: v => <Badge color="navy">{v}</Badge> },
    { key: 'examMode', label: 'Exam', render: v => <Badge color={String(v).toUpperCase() === 'ONLINE' ? 'teal' : 'navy'}>{v}</Badge> },
